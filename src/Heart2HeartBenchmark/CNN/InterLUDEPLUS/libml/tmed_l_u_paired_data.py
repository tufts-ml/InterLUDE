...
class TMED_l_u_paired:
    def __init__(self, l_dataset_path, u_dataset_path, u_batchsize_multiplier, transform_fn=None):
        
        self.l_dataset = np.load(l_dataset_path, allow_pickle=True) #need to use HWC version of the data
        self.u_dataset = np.load(u_dataset_path, allow_pickle=True) #need to use HWC version of the data
