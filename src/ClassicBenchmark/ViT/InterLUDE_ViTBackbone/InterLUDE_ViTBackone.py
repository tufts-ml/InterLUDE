...
parser.add_argument('--train_iterations', default=2**19, type=int, help='total iteration to run, follow previous works')
parser.add_argument('--iteration_per_epoch', default=1024, type=int)
parser.add_argument('--train_epoch', default=512, type=int)
...
parser.add_argument('--lr', default=0.03, type=float, help='learning rate')    #5e-4 for cifar10/100, vit-small; 1e-4 for stl10 vit base
...
parser.add_argument('--temperature', default=1.0, type=float, help='temperature for label guessing')

parser.add_argument('--mu', default=7, type=int,
                    help='coefficient of unlabeled batch size')

parser.add_argument('--threshold', default=0.95, type=float,  #0.95 as default
...
#     paired_l_u_loader = DataLoader(paired_l_u_dataset, args.labeledtrain_batchsize, shuffle=True, drop_last=True, num_workers=args.num_workers, pin_memory=True)
    paired_l_u_loader = DataLoader(paired_l_u_dataset, args.labeledtrain_batchsize, sampler=CustomSampler(paired_l_u_dataset.l_size, args.iteration_per_epoch*args.labeledtrain_batchsize), num_workers=args.num_workers, pin_memory=True)
    test_loader = DataLoader(test_dataset, 128, shuffle=False, drop_last=False, num_workers=args.num_workers, pin_memory=True)
    ...
    if args.lr_schedule_type == 'CosineLR':
        ...
        #scheduler = CosineAnnealingLR(optimizer, T_max = args.train_iterations, eta_min=1e-6, last_epoch=-1, verbose=False)
...
if __name__ == '__main__':
    ...
    if( args.arch == 'vit'):
        if args.dataset_name == 'cifar10' or args.dataset_name == 'cifar100':
            args.scale = 'small'
    experiment_name = temp+'_'+args.arch + '_' + args.scale +"_LambdaRelativeLoss-{}_RlossWarmupType-{}_PN-{}_LambdaUMax-{}_lr-{}_wd-{}".format( args.lambda_relative_loss, args.relativeloss_warmup_schedule_type, args.pn_strength, args.lambda_u_max, args.lr, args.wd)
    ...
    brief_summary['algorithm'] = 'InterLUDE_ViT'
