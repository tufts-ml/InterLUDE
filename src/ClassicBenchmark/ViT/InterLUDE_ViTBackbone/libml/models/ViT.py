...
class Attention(nn.Module):
    ...
    def forward(self, x, flag, strength = 0.0, noise_type = 'linear'):
        
        if(flag):
            hidden_states_copy = x.detach()
            hidden_states_copy = torch.cat( (hidden_states_copy[1:,1:,:], hidden_states_copy[0,1:,:].unsqueeze(0)),dim=0 )
            #x[:,1:,:] = x[:,1:,:] + strength * (hidden_states_copy - x[:,1:,:].detach())
            if(noise_type == 'linear'):
                noise = hidden_states_copy - x[:,1:,:].detach()
                x[:,1:,:] = x[:,1:,:] + strength * noise  #keep class token unchanged 
            else:
...
class Block(nn.Module):
    ...
    def forward(self, x, flag = False, strength = 0.0, noise_type='linear'):  # default: not add noise
        x = x + self.drop_path1(self.ls1(self.attn(self.norm1(x), flag, strength, noise_type)))
...
class NoisyViT(VisionTransformer):
    def __init__(self, patch_size, num_classes, embed_dim, depth, num_heads, img_res, pn_strength ):
        super().__init__(patch_size=patch_size, num_classes=num_classes, embed_dim = embed_dim, depth = depth, num_heads = num_heads, img_size = img_res)
        self.layer = depth  
        self.pn_strength = pn_strength
    
    def forward(self, x, noise_layer_index = 11, noise_type = 'linear'):   #default to the last layer
        x = self.patch_embed(x)
        ...
        for i in range(self.layer):
            if(i == noise_layer_index):
                x = self.blocks[i](x, True, self.pn_strength, noise_type)
            else:
                x = self.blocks[i](x, False, self.pn_strength, noise_type)
        x = self.norm(x)
        x = self.fc_norm(x)
        return self.head(x[:,0])


def build_vit(scale, patch_size, img_res, num_classes, pn_strength):
    ...
    return NoisyViT( patch_size = patch_size,
                      embed_dim  = embed_dim,
                      depth = depth,
                      num_heads = num_heads,
                      img_res = img_res,
