...
#         print('u_strongs shape: {}'.format(u_strongs.shape)) #--torch.Size([64, 7, 3, 32, 32])
        ...
        loss_sat, mask, tau_t, p_t, label_hist, targets_u = sat_criterion(
        ...
        repeated_labels = l_labels.repeat_interleave(args.mu)
...
#             loss -= args.em * ((logits_u.softmax(1) * F.log_softmax(logits_u, 1)).sum(1)).mean()
        ...
        p_bar.set_description("Train Epoch: {epoch}/{epochs:4}. Iter: {batch:4}/{iter:4}. MeanGradientNorm: {MeanGradientNorm:.3f}. LR: {lr:.4f}. Data: {data:.3f}s. Batch: {bt:.3f}s. Loss: {total_loss:.4f}. Loss_x: {labeled_loss:.4f}. Loss_u: {unlabeled_loss_unscaled:.4f}. Loss_saf: {SAF_loss_unscaled:.4f}. tau_t: {tau_t:.2f}. p_t: {p_t:.2f}. Loss_relative: {relative_loss_unscaled:.4f}. Mask: {mask:.2f}. ".format(
                epoch=epoch + 1,
